...
const CustomNumberInput = React.forwardRef(function CustomNumberInput(
  ...
  const handleInputChange = (event) => {
    ...
    // Format the input value with custom comma pattern
  ...
  const formatNumber = (value) => {
    const numericValue = value.replace(/[^\d.]/g, ''); // Remove non-numeric characters except dot

    const [integerPart, decimalPart] = numericValue.split('.'); // Split into integer and decimal parts

    // Format the integer part with commas (groups of 2-3-3)
    let formattedInteger = '';
    let remainingDigits = integerPart.length % 8;

    for (let i = 0; i < integerPart.length; i++) {
      if (i > 0 && (i === 2 || (i > 2 && (i - remainingDigits) % 3 === 0))) {
        formattedInteger += ',';
      }
      formattedInteger += integerPart[i];
    }

    // Reassemble the formatted value with decimal part (if exists)
    const formattedValue = formattedInteger + (decimalPart ? `.${decimalPart}` : '');
